...
public class LessonDetailsResource {
    ...
    public ResponseEntity<LessonDetails> createLessonDetails(@Valid @RequestBody LessonDetails lessonDetails) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/lesson-details/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, result.getId().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /lesson-details/:id} : Updates an existing lessonDetails.
     *
     * @param id the id of the lessonDetails to save.
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/lesson-details/{id}")
    public ResponseEntity<LessonDetails> updateLessonDetails(
        @PathVariable(value = "id", required = false) final Long id,
        @Valid @RequestBody LessonDetails lessonDetails
    ) throws URISyntaxException {
        log.debug("REST request to update LessonDetails : {}, {}", id, lessonDetails);
        if (lessonDetails.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, lessonDetails.getId())) {
            throw new BadRequestAlertException("Invalid ID", ENTITY_NAME, "idinvalid");
        }

        if (!lessonDetailsRepository.existsById(id)) {
    ...
    /**
     * {@code PATCH  /lesson-details/:id} : Partial updates given fields of an existing lessonDetails, field will ignore if it is null
     *
     * @param id the id of the lessonDetails to save.
     * @param lessonDetails the lessonDetails to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated lessonDetails,
     * or with status {@code 400 (Bad Request)} if the lessonDetails is not valid,
     * or with status {@code 404 (Not Found)} if the lessonDetails is not found,
     * or with status {@code 500 (Internal Server Error)} if the lessonDetails couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/lesson-details/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<LessonDetails> partialUpdateLessonDetails(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody LessonDetails lessonDetails
    ) throws URISyntaxException {
        log.debug("REST request to partial update LessonDetails partially : {}, {}", id, lessonDetails);
        if (lessonDetails.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, lessonDetails.getId())) {
        ...
        Optional<LessonDetails> result = lessonDetailsRepository
            .findById(lessonDetails.getId())
            .map(existingLessonDetails -> {
                if (lessonDetails.getContent() != null) {
                    existingLessonDetails.setContent(lessonDetails.getContent());
                }

                return existingLessonDetails;
            })
            .map(lessonDetailsRepository::save);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, lessonDetails.getId().toString())
        );
    }
